...
for dataset, dataframe in pa_dict.items(): 
    ...
    displacement = list(pa_dict[dataset].loc['Max_Displacement/mm'])
...
# plotting_dict['DATASET_22']['displacement'] = [10]*20
# plotting_dict['DATASET_21']['displacement'] = [20]*20
...
annot.connect("add", lambda sel: sel.annotation.set_text(
    f"{plotting_dict[sel.artist.get_label()]['mesh'][sel.target.index]}\n"
    f"Height: {plotting_dict[sel.artist.get_label()]['height'][sel.target.index]}m\n"
    f"Max_Displacement/mm: {plotting_dict[sel.artist.get_label()]['displacement'][sel.target.index]}"
))
