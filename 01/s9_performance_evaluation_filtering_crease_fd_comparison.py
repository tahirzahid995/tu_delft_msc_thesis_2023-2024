...
datasets = ['DATASET_21', 'DATASET_22']
...
for dataset, dataframe in pa_dict.items(): 
    ...
    dataset_file_path = os.path.join(main_path, dataset, '00_geometry_generation\dataset\dataset.csv')
    ...
    plotting_dict[dataset]['fd']= fd_list 
# ############################################################################## AVERAGE DATASET
...
for i, (dataset, data) in enumerate(plotting_dict.items()):
    scatter = ax.scatter(data['fd'], [math.log(i) for i in data['utilization']], color=colors[i], label=dataset)
...
annot.connect("add", lambda sel: sel.annotation.set_text(
    ...
    f"Force density: {plotting_dict[sel.artist.get_label()]['fd'][sel.target.index]}m\n"
    f"Log(Utilization): {plotting_dict[sel.artist.get_label()]['utilization'][sel.target.index]}"
))

# Add labels
plt.xlabel('Force Density')
plt.ylabel('Log(Utilization)')
...
scatter_handles = []
for i, (dataset, data) in enumerate(plotting_dict.items()):
    scatter = ax.scatter(data['fd'], data['buckling'], color=colors[i], label=dataset)
    scatter_handles.append(scatter)
...
scatter_handles = []
for i, (dataset, data) in enumerate(plotting_dict.items()):
    scatter = ax.scatter(data['fd'], data['displacement'], color=colors[i], label=dataset)
    scatter_handles.append(scatter)
